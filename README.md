...
- **6 game factory agents**: game-factory (master pipeline, weekly Saturday 04:00), production-bridge, game-runner, auto-playtest, visual-qa, refine-loop — autonomous end-to-end pipeline that chains design, code generation, playtesting, visual QA, and iterative refinement
...
└──────────────────────┬──────────────────────────────────┘
                       │ HTTP / SSE / WebSocket
┌──────────────────────▼──────────────────────────────────┐
│  Python Skill Runtime (~92 agents)                       │
│                                                         │
...
| **Dev Automation** | KAELEN | The Iron Codex | 9 | Repo monitoring, refactoring, artifact lifecycle, dev digest, auto-fix, CI runner, regression guard |
...
| **Game Dev** | ARDENT | The Eternal Engine | 29 | Game design, mechanic prototyping, balance audits, level design, playtesting, asset integration, code generation, code review, testing, build pipeline, scene assembly, character design, item/weapon forging, enemy/boss design, quest writing, story/lore writing, skill tree building, asset requesting, project initialization, automated iteration, data population, **game-factory autonomous pipeline**, production bridge, game runner, auto-playtest, visual QA, refine loop |
...
| Market Data | Tradovate (free CME futures via prop firm), Databento (paid), Twelve Data (free, 3yr+ 1-min ETFs), Polygon.io (free tier, international), Alpaca (free ETF), CSV imports, yfinance (fallback) |
...
| **Twelve Data** | 1m-1d | ETFs (SPY/QQQ/GLD/DIA), 3yr+ 1-min history | Free (800 credits/day) | Best free option for daytrading backtesting |
| **Polygon.io** | 1m-1d | Stocks, ETFs, CME futures | Free tier (5 calls/min) | Works internationally (Canada) |
| **Alpaca** | 1m-1d | Proxied via ETFs (SPY/QQQ/GLD/DIA/USO/TLT) | Free | Good free option without prop firm account |
...
Provider selection is automatic: tradovate > databento > twelvedata > polygon > alpaca > csv > yfinance. Set `MARKET_DATA_PROVIDER` in `.env` to override. All providers return the same OHLCV format — the trading pipeline doesn't change.
...
2. Create an application → note the App ID and CID
3. Add to `.env`: `TRADOVATE_USERNAME`, `TRADOVATE_PASSWORD`, `TRADOVATE_APP_ID`, `TRADOVATE_CID`
4. Set `TRADOVATE_DEMO=true` for evaluation accounts, `false` for funded
5. All trading skills automatically switch to real CME futures data
